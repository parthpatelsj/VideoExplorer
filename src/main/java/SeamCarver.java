...
//        result = runSeamCarve(image);
//
//        try{
//            File output = new File("testing.png");
//            ImageIO.write(result, "png", output);
//        } catch (IOException e) {
//            System.err.println("Trouble saving");
//        }
//    }

    public static BufferedImage runSeamCarve(BufferedImage in) {

        Integer horizontalNum = null;
        Integer verticalNum = null;
        ...
        Set<Point> facePoints = fd.getFacePoints(in);
        while (verticalNum > 0) {
            System.out.println("Vertical Carving: " + verticalNum);
            // Get the new image w/o one seam.
            newImage = carveSeam(facePoints, newImage, "vertical");

            verticalNum = verticalNum - 1;
        }

        while (horizontalNum > 0) {
            System.out.println("Horizontal Carving: " + horizontalNum);
            // Get the new image w/o one seam.
            newImage = carveSeam(facePoints, newImage, "horizontal");
        ...
        try {
            File outputfile = new File("seamCarved.png");
     ...
     * @return carved image.
     */
    private static BufferedImage carveSeam(Set<Point> facePoints, BufferedImage image, String direction) {
        // We need to compute the energy table, find and remove a seam.
        BufferedImage newImage = null;
        double[][] energyTable = computeEnergy(facePoints, image);
        int[][] seam = findSeam(energyTable, direction);
        newImage = removeSeam(facePoints, image, seam, direction);
     ...
     */
    private static double[][] computeEnergy(Set<Point> facePoints, BufferedImage image) {
...
//                System.out.println(energy);
                ...
                if(facePoints.contains(new Point(x, y))) {
                    System.out.println("Pixel (" + x + "," + y +") is part of a face!");
                    energyTable[x][y] = 999999.9;
                }
     ...
     * @return carved image.
     */
    private static BufferedImage removeSeam(Set<Point> facePoints, BufferedImage image, int[][] seam, String direction) {
...
//                    if(facePoints.contains(new Point(x, y))) {
//                        System.out.println("Pixel is part of a face!");
//                        inSeam = false;
//                        shift = false;
//                    }
